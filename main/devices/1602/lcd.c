...
void LCDI2C_clear(){
	...
	os_sleep_ms(3);  // this command takes a long time!
}
...
void LCDI2C_expanderWrite(uint8_t _data){
    ...
    _data|=lcdi2c.backlightval;
    I2C_Write(lcdi2c.Addr,&_data,1);
}

void LCDI2C_pulseEnable(uint8_t _data){
	LCDI2C_expanderWrite(_data | En);	// En high
	yield();		// enable pulse must be >450ns

	LCDI2C_expanderWrite(_data & ~En);	// En low
	os_sleep_ms(1);		// commands need > 37us to settle
}
